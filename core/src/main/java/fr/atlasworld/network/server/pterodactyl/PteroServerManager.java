...
public class PteroServerManager implements ServerManager {
    ...
    public void initialize() throws PanelException {
        ...
        if (serverConfigurationsFiles == null || serverConfigurationsFiles.length < 1) {
            AtlasNetworkOld.logger.warn("No server configurations found, AtlasNetwork will not be able to create servers automatically");
        } else {
            for (File serverConfig : serverConfigurationsFiles) {
                ...
                if (serverConfigurations.containsKey(configuration.id())) {
                    AtlasNetworkOld.logger.warn("Found multiple '{}' skipping {}", configuration.id(), serverConfig.getName());
                    continue;
                }
        ...
        try {
            for (ApplicationServer server : networkManagedServers) {
                ...
                if (optStoredServer.isEmpty()) {
                    AtlasNetworkOld.logger.warn("Found unknown server '{}', Deleting server..", server.getName());
                ...
                if (serverUses.getState() == UtilizationState.OFFLINE || serverUses.getState() == UtilizationState.STOPPING) {
                    AtlasNetworkOld.logger.info("'{}' is offline, deleting unused server..", server.getName());
                    server.getController().delete(false).executeAsync();
                ...
                if (!this.serverConfigurations.containsKey(storedServer.getType())) {
                    AtlasNetworkOld.logger.warn("Found '{}' with an out-dated configuration id '{}', deleting server..", server.getName(), storedServer.getId());
                    server.getController().delete(false).executeAsync();
                    this.serverDatabase.remove(storedServer.getId());
                ...
                CompletableFuture.runAsync(() -> {
                    try {
                        AtlasNetworkOld.SOCKET_STARTED_LATCH.await();
                        panelServer.getClientServer().sendCommand("network reconnect").execute();
                        AtlasNetworkOld.logger.info("Sent reconnect request to {}", panelServer.name());
                    } catch (InterruptedException e) {
                        AtlasNetworkOld.logger.error("Could not wait on the socket!", e);
                    }
                });
        ...
        if (server == null) {
            AtlasNetworkOld.logger.info("No Server found, creating a new one..");
        ...
        if (proxy == null) {
            AtlasNetworkOld.logger.info("No Proxy found, creating a new one..");
    ...
    public PanelServer createDefaultProxy(UUID requestId) throws PanelException {
        ...
        try {
            ...
            AtlasNetworkOld.logger.error("Could not add proxy to load balancer!");
            server.delete();
            try {
