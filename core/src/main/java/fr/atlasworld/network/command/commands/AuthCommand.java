...
public class AuthCommand {
    ...
    public static int executeListProfiles(CommandSource source) {
        try {
            Set<AuthenticationProfile> profiles = AtlasNetworkOld.getDatabaseManager().getAuthenticationProfileDatabase()
                    .getAllEntries();
    ...
    public static int executeCreateProfile(CommandSource source, UUID uuid) {
        try {
            Database<AuthenticationProfile> database = AtlasNetworkOld.getDatabaseManager().getAuthenticationProfileDatabase();
            ...
            String token = AtlasNetworkOld.getSecurityManager().generateAuthenticationToken();
            AuthenticationProfile profile = new AuthenticationProfile(uuid.toString(), AtlasNetworkOld.getSecurityManager().hash(token));
